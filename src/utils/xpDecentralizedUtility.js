...
export class XPDecentralizedUtility {
    ...
    lockNFT = async(fromChain, toChain, nft, receiver, fee=null, discountLeftUsd=null, account=null) => {
        if(this.isV3Enabled){
           return await this.lockNFT_V3(fromChain, toChain, nft, receiver);
        }
        else{
            return await this.lockNFT_V2(fromChain, toChain, nft, receiver, fee, discountLeftUsd, account)
        }
    }

    lockNFT_V2 = async(fromChain, toChain, nft, receiver, fee, discountLeftUsd, account) => {
    ...
    lockNFT_V3 = async(fromChain, toChain, nft, receiver, ) => {
        ...
        const { tx: result, lockNftMintWith: mintWith } = res;
    ...
    getClaimData = async(originChain, hash) => {
        ...
        console.log("receipt**********",await originChain.getProvider().getTransactionReceipt(hash));
    ...
    claimNFT = async(originChainIdentifier, targetChainIdentifier, hash, chainWapper=null, fromChainWapper=null) => {
        if(this.isV3Enabled){
            return await this.claimNFT_V3(originChainIdentifier, targetChainIdentifier, hash);
        }
        else{
    ...
    claimNFT_V3 = async(originChainIdentifier, targetChainIdentifier, hash) => {
        ...
        const signatures = await this.getLockNftSignatures(targetChain, hash, originChainIdentifier)

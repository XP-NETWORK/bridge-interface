...
export default function NFTcard({
  ...
  const getDetails = async (nft) => {
    ...
    if (!nftState.image && !nftState.animation_url && nftState.uri) {
      ...
      // fetch("https://murderheaddeathclub.com/api/nft/816", {
      //   headers: {
      //     "Access-Control-Allow-Origin": "*",
      //     'Content-Security-Policy': 'upgrade-insecure-requests'
  ...
  const onClickWhiteListButton = async () => {
    ...
    try {
      ...
      const interval = setInterval(
        () =>
          bridgeWrapper.isWhitelisted(from.nonce, nftState).then((result) => {
            if (result) {
              ...
              whitelistedPool.whitelistContract(nftState.native.contract);
      ...
      <div className={`nft-box__wrapper`} ref={cardRef}>
        {!nftState?.dataLoaded ? (
          ...
          <div
            ...
            className={
              nftState.whitelisted ? "nft__card--selected" : "nft__card"
            }
          >
            ...
            <div className="nft__main">
              ...
              {nftState.uri && nftState.image && nftState.animation_url ? (
                ...
                <Image onError={setImageErr} nft={nftState} index={index} />
              ...
              {claimables && <ClaimableCard nft={nftState} index={index} />}
            </div>
            <div className="nft__footer">
              ...
              <span className="nft-number">{nftState.native?.tokenId}</span>
            </div>
          </div>
        )}
      </div>

...
function SendFees(props) {
  ...
  async function estimate(fromChain, toChain) {
    ...
    if (from.type === "Elrond") {
      dispatch(setBigNumFees(fees * selectedNFTList.length));
    } else {
    ...
    nfts = nfts.reduce(
      (acc, nft) => {
        return {
          ...
          usedContracts: acc.usedContracts.concat(nft.native.contract),
        };
      },
      {
        items: [],
    ...
    <div className="fees__container">
      ...
      {deployFees && selectedNFTList?.length ? (
        <div className="fees deploy-fees">
          ...
          <div>
            <span>{deployFees.toFixed(getNumToFix(deployFees))}</span>

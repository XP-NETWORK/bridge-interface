...
const NFTmobileView = ({ selectedNFTs, _from, nfts }) => {
	...
	// const _nfts = useSelector((state) => state.general.NFTList);
			...
			<div className="mobile-nfts__list">
				{!showNFTsSearch ? (
					<div className="mobile-nfts__header">
						<SelectedNFTs
							on={true}
							show={selectedNFTs.length > 0 ? handleShowSelected : undefined}
							// show={true}
							showSelected={showSelected}
							setOff={setShowSelected}
						/>
						{_from.type === "EVM" && nfts?.length < 1 && <ImportNFTButton />}
						{nfts?.length > 0 && (

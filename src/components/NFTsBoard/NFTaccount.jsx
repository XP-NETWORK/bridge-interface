...
function NFTaccount(props) {
    ...
    async function getNFTsList(fromChain) {
        ...
        try {
            let nfts = await fromChain.getNFTs(bridge.checkWallet || account);
    ...
    const getBalance = async (fromChain) => {
        const _balance = await fromChain.balance(account);
    ...
    useDidUpdateEffect(() => {
        if (nfts.length) {
            saveForSearch(account, _from.nonce, nfts);
        }
    }, [nfts]);
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            chainSpecific && chainSpecific(dispatch, fromChain, account);

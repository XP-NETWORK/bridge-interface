              ...
              {accounts && (
                <ul className="algoSigner__accounts">
                  {accounts?.map((account) => (
                    <li
                      onClick={() => setAlgoAccountToOptIn(account)}
                      className="algo-opt-in__button"
                      key={`${account["address"]}`}
                    >{`${account["address"].substring(0, 4)}...${account[
                      "address"
                    ].substring(account["address"].length - 4)}`}</li>
                  ))}
                </ul>
              )}
            </div>
          ...
          <Modal.Body>
            <div className="walletListBox">
              ...
              <div className="algo-opt-in__btns">
                <div onClick={handleClose} className="algo-opt-in__button">
                  Skip for now
                </div>
                <div
                  // onClick={optIn}

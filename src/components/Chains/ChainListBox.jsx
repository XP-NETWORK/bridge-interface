...
export default function ChainListBox(props) {
    ...
    useEffect(() => {
        ...
        const onlyElrond = elrondAccount
            ? sorted.filter((chain) => chain.type === "Elrond")
            : undefined;
        const onlyVeChain =
            evmAccount && Sync2
                ? sorted.filter((chain) => chain.type === "VeChain")
                : undefined;
        const onlyEVM = evmAccount
            ? sorted.filter((chain) => chain.type === "EVM")
            : undefined;
        const onlyTron = tronAccount
            ? sorted.filter((chain) => chain.type === "Tron")
            : undefined;
        const onlyAlgo = algorandAccount
            ? sorted.filter((chain) => chain.type === "Algorand")
            : undefined;
        const onlyTezos = tezosAccount
            ? sorted.filter((chain) => chain.type === "Tezos")
            : undefined;
        const set =
            onlyVeChain ||
            onlyElrond ||

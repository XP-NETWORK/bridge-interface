...
const settingsHoc = (Wrapped) =>
  function CallBack() {
    ...
    const [toggleEditor] = useState(false);
    const [debouncedAcc] = useState({
      key: "",
      val: "",
    });
      ...
      () =>
        wid
          ...
          : `${window.location.href
              ...
              checkRgbaOut(btnBackground).split(
                "#"
              )[1]}&btnRadius=${btnRadius &&
    ...
    const onSaveSettings = async () => {
      ...
      if (wid) {
        try {
          await wservice.update(wid, newSettings);

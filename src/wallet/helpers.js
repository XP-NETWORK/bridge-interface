...
export const parseEachNFT = async (nft, index, testnet, claimables) => {
  ...
  } else if (collectionIdent === "0x691bd0f2f5a145fcf297cf4be79095b66f002cbc") {
    uri = `https://api.crosspunks.com/cars/meta/2/${nft.native.tokenId}`;
  } else if (collectionIdent === "0x7f3495cf2d05db6e9e52cdf989bced71e786725c") {
    uri = `https://api.crosspunks.com/cars/meta/1/${nft.native.tokenId}`;
  } else if (collectionIdent === "0x36f8f51f65fe200311f709b797baf4e193dd0b0d") {
    uri = `https://treatdao.com/api/nft/${nft.native.tokenId}`;
  }
  ...
  if (from.text === "Tezos") {
    ...
    nftObj.animation_url = videoFormat
      ? nftObj.animation_url || nft.native?.meta?.token?.metadata?.formats?.uri
      : undefined;
    ...
    if (typeof data === "object") {
      ...
      if (!data?.includes("json")) {
        ...
        else if (data?.includes("image")) nftObj.animation_url = setupURI(uri);
      } else if (data) {
        let n;
        try {
  ...
  } else if (collectionIdent === "0xf0E778BD5C4c2F219A2A5699e3AfD2D82D50E271") {
    const { data } = await axios(setupURI(nft.uri));
      ...
      return await new Promise(async (resolve) => {
        try {
          ...
          if (res && res.data) {
            ...
            let result =
              typeof res.data != "string" ? { ...res.data, ...n } : { ...n };
            if (isImageIPFS) {
